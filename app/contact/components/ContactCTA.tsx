  ...
  return (
    <section className="py-20 px-6 bg-primary relative overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-primary-foreground mb-6">Ready to Start Your Project?</h2>
          <p className="text-xl text-primary-foreground/80 max-w-3xl mx-auto mb-8">
            Join 500+ satisfied clients who have transformed their ideas into successful digital products with Oriums.
          </p>

          {/* Trust Indicators */}
          <div className="flex flex-wrap justify-center items-center gap-8 text-primary-foreground/80 mb-12">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {ctaOptions.map((option, index) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
            >
              <div className={`inline-flex p-4 rounded-full mb-4 ${option.primary ? "bg-primary/10" : "bg-primary-foreground/20"}`}>
                <option.icon className={`h-8 w-8 ${option.primary ? "text-primary" : "text-primary-foreground"}`} />
              </div>
              <h3 className={`text-xl font-semibold mb-2 ${option.primary ? "text-card-foreground" : "text-primary-foreground"}`}>
                {option.title}
              </h3>
              <p className={`mb-6 ${option.primary ? "text-muted-foreground" : "text-primary-foreground/80"}`}>{option.description}</p>
              <Button
        ...
        >
          <div className="bg-primary-foreground/10 backdrop-blur-sm border border-primary-foreground/20 rounded-2xl p-8 max-w-2xl mx-auto">
            <h3 className="text-2xl font-semibold text-primary-foreground mb-4">Not sure where to start?</h3>
            <p className="text-primary-foreground/80 mb-6">
              Download our free project planning guide and get a clear roadmap for your digital transformation.
            </p>
            <Button variant="outline" className="bg-primary-foreground/20 hover:bg-primary-foreground/30 text-primary-foreground border-primary-foreground/30 gap-2">
              📥 Download Free Guide
            </Button>
          </div>
        </motion.div>
